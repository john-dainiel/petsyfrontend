...
function initRunner() {
  ...
  function loadImage(img, src) {
    return new Promise(resolve => {
      img.src = src;
      img.onload = () => { imagesLoaded++; resolve(); };
  ...
  function spawnCoin() {
    coins.push({x: Math.random()*(canvas.width-40), y: -30, width:40, height:40});
  }
  ...
  document.getElementById('runnerStartBtn')?.addEventListener('click', async ()=>{
    await Promise.all([
    ...
    if(imagesLoaded<TOTAL_IMAGES){ alert('Loading images, wait...'); return; }
    startRunnerGame();
  });
  ...
  document.getElementById('quizStartBtn')?.addEventListener('click',()=>{
    quizStarted=true; document.getElementById('quizStartBtn').disabled=true; showQuizQuestion();
  });
  ...
  const memoryImages=['/static/images/memory1.png','/static/images/memory2.png','/static/images/memory3.png',
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  loadPlayerInfo();
  initRunner();
  // initQuiz(); initMemory(); <- call your quiz & memory init functions here similarly
