...
function initRunner() {
  ...
  function loadImage(img, src) {
    return new Promise(resolve => { img.src = src; img.onload=resolve; img.onerror=resolve; });
  }
...
function initQuiz() {
  quizCoins=0; currentQuestion=null;
...
function showQuizControls() {
  ...
  const playAgain=document.createElement('button'); playAgain.className='quiz-control-btn'; playAgain.innerText='▶ Play Again'; playAgain.onclick=showQuizQuestion;
  const stop=document.createElement('button'); stop.className='quiz-control-btn stop'; stop.innerText='⏹ Stop';
  stop.onclick=()=>{ 
...
document.getElementById('quizStartBtn')?.addEventListener('click',()=>{
  showQuizQuestion(); document.getElementById('quizStartBtn').disabled=true;
});

initQuiz();

/* ==================== MEMORY GAME ==================== */
let memoryCards=[], memoryFlipped=[], memoryMatched=[], memoryLevel=1, memoryCoins=0, memoryTime=30, memoryTimer=null;
const memoryImages=[
...
function initMemory() {
  clearInterval(memoryTimer); memoryLevel=1; memoryCoins=0; memoryCards=[]; memoryFlipped=[]; memoryMatched=[]; memoryTime=30;
...
function startMemoryLevel() {
  ...
  memoryTime=Math.max(10,30-(memoryLevel-1)*3);
  renderMemory();
  memoryTimer=setInterval(()=>{
    memoryTime--; document.getElementById('memoryTimer').innerText=`⏱️ ${memoryTime}s`;
    if(memoryTime<=0){
      clearInterval(memoryTimer); showPopup(`⏰ Time's up!<br>Coins earned: 🪙 ${memoryCoins}`,()=>initMemory());
